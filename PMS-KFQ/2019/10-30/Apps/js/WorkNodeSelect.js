...
var onInActiveClick = function (id) {
    ...
    for (var iTmp = 0; iTmp < len; iTmp++) {
        ...
        if ("check" + tmpNodeCode == id)
            lineType = AllData.NextNodeList[iTmp].LineType;
    }
    //如果当前是排他线，则取消其他节点的勾
    if (lineType == 20 && $('#' + id).children('span').is(".fa-toggle-on")) {
        var len = AllData.NextNodeList.length;
        for (var iTmp = 0; iTmp < len; iTmp++) {
            var tmpNodeCode = AllData.NextNodeList[iTmp].NodeCode;
            if ("check" + tmpNodeCode == id) continue;
            $('#check' + tmpNodeCode).children('span').removeClass('fa-toggle-on');
            $('#check' + tmpNodeCode).siblings('.slidedivs').slideUp(500);
        }
    }
}
...
var getSelectedInfo = function (currentInfo) {
    ...
    for (var iTmp = 0; iTmp < len; iTmp++) {
        ...
        //var listCopyBox = mini.get("lstCopyNode" + nodeCode);
        //如果当前节点没有展开，直接忽略
        if (!$('#check' + nodeCode).children('span').is(".fa-toggle-on")) {
            continue;
        }
