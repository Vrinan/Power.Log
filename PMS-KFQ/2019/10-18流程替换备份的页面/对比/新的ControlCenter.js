var ControlCenter = function () {
    ...
    //提取流程列表
    var onSubmitDatas = function (flowOperate) {
        var self = this;
        ...
        $.ajax({
            ...
            success: function (text) {
                ...
                if (!tmpResult) {
                    alert("严重异常，返回对象不可访问"); postInfo = {}; return false;
                }
                if (tmpResult.success == false) {
                    alert(tmpResult.message); postInfo = {}; return false;
                }
                var myFrame = null;
    ...
    var onSaveForm = function (data, flowOperate, validMustFill) {
        ...
        return onSubmitDatas(flowOperate);
    ...
    var onEndExecute = function (flowOperate, result) {
        ...
        return onSubmitDatas(flowOperate);  //执行
    } 
    var execute = function (curFlowOperate,  validMustFill) {
        var flow = new WorkFlowsForm();
        flow.setCallBack(onEndExecute);  //绑定回调函数
        ...
        switch (curFlowOperate) {
            case EFlowOperate.Update:
                var res = onSaveForm(postData, curFlowOperate, validMustFill);
                if (res == false) {
                    if (CurrentSender && typeof (CurrentSender.setEnabled) == "function") CurrentSender.setEnabled(true);
                    return;
                }
                break;
            case EFlowOperate.ReadFlow:  //加载流程信息  
                flow.onReadFlowInfo(postData, Events, curFlowOperate);
                break;
            ...
            case EFlowOperate.ShowHistoryMonitor:  //监控历史  
                flow.onExecute(postData, Events, curFlowOperate);
                break;
    ...
    return myCenterSelf = {
        ...
        onExecute: function (sender, data, events, flowOperates) {
            ...
            if (FlowOperates.length > 1) NextOperate = FlowOperates[1];//第二个指令
            ...
            execute(FlowOperates[0], validMustFill);
        },
