 ...
 //转发人员
    ...
    return {
        ...
        SelectSendNode: function (data, events, flowOperate) {
            ...
            var win = mini.open({
                ...
                ondestroy: function (action) {
                    ...
                    result.data = msg;
                    if (CallBackFunction){
                        CallBackFunction(flowOperate, result);
                        //赋值吧KeyWord传到ControlCenter，用于关闭页面-Aladd 181011
                        result.data.data.Current.KeyWord = data.KeyWord;
                    } 
                    if (PowerForm && PowerForm.EventSendNodeEnd) { 
                        PowerForm.EventSendNodeEnd(result); 
                    }
        ...
        ShowMonitor: function (data, events, flowOperate) {
            ...
            if (workflowdata && workflowdata.CanFlowOperate && workflowdata.CanFlowOperate.CanFlowList
                && workflowdata.CanFlowOperate.CanFlowList.ShowMonitor
                && workflowdata.CanFlowOperate.CanFlowList.ShowMonitor.Title =="HisDataNodes") {
                mini.open({
