    ...
    return funsself = {
        ...
        WorkFlowHandle: function () {
            ...
            if (workflowdata.WorkFlowFlag && workflowdata.WorkFlowFlag == "workflows") {
                ...
                //是当前审批人，且设置了审批人可修改表单
                if (workflowdata.CanFlowOperate.UserType == EUserType.HandleMan && enableform != undefined && enableform == 1)
                    return true;
                else
                    return false;
            }
        ...
        LoadRight: function (miniid, bchild) {
            ...
            //Aladd 2019年8月20日 16:31:52
            //重新提醒
            //只有审批中的能看到
            if($("#ReRemind").length==0 && formconfig.FormState!="add" 
                // && formconfig.config.joindata.currow["Status"] != "0" 
                // && formconfig.config.joindata.currow["Status"] != "40" 
                // && formconfig.config.joindata.currow["Status"] != "50"
                ){
                $("#btnForward").parent().append("</i><a class='mini-button flowbuttoncolor' iconcls='fa fa-trash-o' id='ReRemind' onclick='Remind("+config.KeyWord+")'>重新提醒</a>"); 
                $("#ReRemind").show();
            }

            //zyx
            if($("#zyxReturnBtn").length==0 && formstate!="add"){
                $("#btnForward").parent().append("</i><a class='mini-button flowbuttoncolor' iconcls='fa fa-trash-o' id='zyxReturnBtn' onclick='zyxReturnBtn("+config.KeyWord+")'>删除流程</a>"); 
        ...
        OnAddForm: function (formid, defaultdata, grid, params) {
            ...
            var iw = formconfig.config.openwidth || 1000;
        ...
        ExportTreeDataToXls: function (gridid, filename, treefield, groupfield, fnonbeforeload, headercolor, headerfontcolor, exporttype) {
            ...
            var encodejson = encodeURIComponent(mini.encode(json));
            if (exporttype == "pdf") {
                postDataToServer("/Form/ExportTreePageData", { json: encodejson }, "_blank");
            }
            else
                postDataToServer("/Form/ExportTreePageData", { json: encodejson });
        },
...
--再执行更新
select en.Description,en.KeyWord,tb.TableName into BusinessTableName
from Lz_PowerPlat.dbo.pp_Entity en left join Lz_PowerPlat.dbo.pp_EntityTable tb on en.EntityID=tb.EntityID
...
//重新提醒
function Remind(KeyWord)
{
    Power.ui.confirm("系统将发送短信提醒，确定执行吗？", function (action) {
        if (action){
            var dt={"KeyValue":KeyValue,
                    "HumanId":sessiondata.HumanId,
                    "HumanName":sessiondata.HumanName,
                    "Title":formconfig.config.joindata.currow["Title"],
                    "KeyWord":KeyWord.id};
            $.ajax({
                url: "/ALLZ/Remind",
                data: dt,
