...
function GooFlow(bgDiv, property) {
    ...
    if (property.haveHead) {
        ...
        for (var x = 0; x < property.headBtns.length; ++x) {
            var _icon = property.headBtns[x];
            if (typeof (_icon) == "string")
                tmp += "<button class='GooFlow_head_btn btn blue'><i class='ico_" + property.headBtns[x] + "'></i></button>"
            else if (typeof (_icon) == "object") {
                tmp += "<button id='toolbar_" + (_icon.id || "") + "' class='GooFlow_head_btn btn blue " + (_icon.id || "") + "'>"
                if (_icon.icon) {
                    tmp += "<i class='ico_" + _icon.icon + "'></i>";
                }
                tmp += _icon.text + "</button>";
            }
        }
        tmp += "</div>";
        this.$head = $(tmp);
        ...
        if (property.headBtns)
            this.$head.on("click", { inthis: this }, function (e) {
                ...
                else if (tar.tagName == "a" || tar.tagName == "button") tar = tar.childNode[0];
                var This = e.data.inthis;
                var btnName = tar.id.replace("toolbar_", "");
                //定义顶部操作栏按钮的事件
                switch (btnName) {
                    case "ico_new":
                    case "new":
                    ...
                    case "ico_reload":
                    case "reload":
                        if (This.onFreshClick != null) This.onFreshClick(); break;
                    default:
                        var _btns = $.grep(property.headBtns, function (item) {
                            return item.id == btnName
                        });
                        if (_btns.length > 0) {
                            _btns[0].click && _btns[0].click(This);
                        }
                        break;
                }
            });
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            var nodeCodePrev = This.$id;   //连线前缀
            var isFromInstance = false;   //是否流转时追加的节点或连线
            if (WorkFlowMode == "WorkingDesign") { nodeCodePrev = "instance"; isFromInstance = true; }

            //简化版
            var _maxid = 0;
            for (var f in This.$nodeDom) {
                var _node_id = f.replace(nodeCodePrev + "_node_", "");
                if (_node_id) _node_id = parseInt(_node_id);
                if (_node_id > _maxid)
                    _maxid = _node_id;
            } 
            This.$max = _maxid + 1;
            /*  克版
            if (This.$nodeCount > 0 && This.$max == 1) {
                var _max = This.$nodeCount;
                var _id=This.$id + "_node_" + _max;
                if (This.$nodeDom[_id]) {
                    var _maxid = 0;
                    for (var f in This.$nodeDom) {
                        var _node_id = f.replace(This.$id + "_node_", "");
                        if (_node_id) _node_id = parseInt(_node_id);
                        if (_node_id > _maxid)
                            _maxid = _node_id;
                    }
            ...
            */
            var nodeText = "node_" + This.$max;
            if (type.split(" ")[0] == "start") {
                nodeText = "开始";
            }
            if (type.split(" ")[0] == "end") {
                nodeText = "结束";
            ...
            This.addNode(nodeCodePrev + "_node_" + This.$max, { name: nodeText, left: X, top: Y, type: This.$nowType, "IsFromInstance": isFromInstance,"IsInstanced":false });  //新加的节点，肯定没有走实例
            This.$max++; 
        });
        //划线或改线时用的绑定
        ...
        this.$lineOper = $("<div class='GooFlow_line_oper' style='display:none'><i class='b_l1'></i><i class='b_l2'></i><i class='b_l3'></i><i class='b_x'></i><i class='ico_node'></i></div>"); //选定线时显示的操作框
        ...
        this.$lineOper.on("click", { inthis: this }, function (e) {
            ...
            switch ($(e.target).attr("class")) {
                case "b_x":  //删除连线
                    //验证连线是否能删除，能删则删，否则在 CheckLine的时候，设置禁用。 WSL
                    if (This.CheckLineDelete(id) == true) {
                        This.delLine(id);
                ...
                case "ico_node":   //定义连线配置
                    This.popLinePropertyWindow(id, "");
                    break;             
            }
        });
...
GooFlow.prototype = {
    ...
    initDraw: function (id, width, height) {
        ...
        $(this.$draw).delegate(tmpClk, "dblclick", { inthis: this }, function (e) {
            ...
            This.$textArea.val(oldTxt).css({
                display: "block", width: 120, height: 20,
    ...
    addNode: function (id, json) {
        ...
        var _selfIcon = "";
        if (json.iconCls && json.iconCls != "") {
            var _iconStyle = json.iconStyle || "";
            _selfIcon = "<span style='" + _iconStyle+"'><i class='fa " + json.iconCls + "'></i></span>";
        }
        if (!json.type) json.type = json.NodeType;
        if (json.type.indexOf(" round") < 0) {
            ...
            this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'><i class='ico_" + json.type + "'></i>" + _selfIcon+"</td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
        }
        else {
            json.width = 24; json.height = 24;
            this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='0'><tr><td class='ico'><i class='ico_" + json.type + "'></i>" + _selfIcon+"</td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
        }
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".ico", "mousedown", { inthis: this }, function (e) {
            ...
            var _allowDraggable = This.$editable && (This.$nodeData[id].allowDraggable !== false);  //!== 是 强制不等，
            if (!_allowDraggable)
                return false;
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
            ...
            var lineCodePrev = This.$id;   //连线前缀
            var isFromInstance =false;   //是否流转时追加的节点或连线
            if (WorkFlowMode == "WorkingDesign") { lineCodePrev = "instance";isFromInstance=true;}

            if (lineStart && !This.$mpTo.data("p")) {
                
                //简化版
                var _maxid = 0;
                for (var f in This.$lineDom) {
                    var _line_id = f.replace(lineCodePrev + "_line_", "");
                    if (_line_id) _line_id = parseInt(_line_id);
                    if (_line_id > _maxid)
                        _maxid = _line_id;
                }
                This.$max = _maxid + 1;
                ...
                This.addLine(lineCodePrev + "_line_" + This.$max, { from: lineStart.id, to: this.id, name: "", "IsFromInstance": isFromInstance, "IsInstanced": false });  //新加的节点，肯定没有走实例
                This.$max++;
            }
            else {
                if (lineStart) {
    ...
    focusItem: function (id, bool) {
        ...
        if (jq.prop("tagName") == "DIV") {
            ...
            //应吴石磊要求 给节点增加 allowRemove 属性,标记该节点是否允许删除,如果不赋值的话,和之前一样 根据$editable 来判断是否允许删除
            var _allowRemove = this.$editable && (this.$nodeData[id].allowRemove!==false);
            if (_allowRemove) jq.children("div:eq(0)").css("display", "block");
            ...
            //连线图标总是可见， 但是否可删除 ，在 workflowdesign.html 的 checkline 中判断
          //  var _allowRemove = this.$editable && (this.$lineData[id].allowRemove !== false);
          //  if (!_allowRemove) return;
            var x, y, from, to, n;
            if (GooFlow.prototype.useSVG != "") {
    ...
    //删除结点
    delNode: function (id, forceDelete) {
        if (!this.$nodeData[id]) return;
        if (!forceDelete && this.onItemDel != null && !this.onItemDel(id, "node")) return;
        //先删除可能的连线
    ...
    loadData: function (data) {
        ...
        for (var i in data.NodeList)
            this.addNode(i, data.NodeList[i]);
        for (var j in data.LineList) 
            this.addLine(j, data.LineList[j]);
    ...
    exportData: function () {
        var ret = { title: this.$title, NodeList: this.$nodeData, LineList: this.$lineData, areas: this.$areaData, initNum: this.$max };
        for (var k1 in ret.NodeList) {
            if (!ret.NodeList[k1].marked) {
                delete ret.NodeList[k1]["marked"];
            }
        }
        for (var k2 in ret.LineList) {
            if (!ret.LineList[k2].marked) {
                delete ret.LineList[k2]["marked"];
            }
        }
        return ret;
    },
    ...
    clearData: function () {
        ...
        for (var key in this.$lineData) {
            this.delLine(key, true);
        }
        for (var key in this.$areaData) {
            this.delArea(key, true);
        }
        this.$deletedItem = {};
    },
    //销毁自己
    ...
    addLine: function (id, json) {
        ...
        if (json.type) {
            ...
            if (json.M) this.$lineData[id].M = json.M;    //wsl 追加，否则会出现 undefined, 原因不详
        }
        else
    ...
    //删除转换线
    delLine: function (id, forceDelete) {
        if (!this.$lineData[id]) return;
        if (!forceDelete && this.onItemDel != null && !this.onItemDel(id, "node")) return;
        if (this.$undoStack) {
    ...
    markItem: function (id, type, mark) {
        if (type == "node") {
            ...
            if (GooFlow.prototype.useSVG != "") {
                if (mark) {
                    this.$lineDom[id].childNodes[1].setAttribute("stroke", GooFlow.prototype.color.mark || "#ff3300");
    ...
    //删除区域分组
    delArea: function (id, forceDelete) {
        if (!this.$areaData[id]) return;
        if (this.$undoStack) {
