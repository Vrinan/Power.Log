...
var bindNode = function (data) {
    ...
    for (var iTmp = 0; iTmp < len; iTmp++) {
        ...
        for (var iRow = 0; iRow < node.CanSelectUsers.length; iRow++) {
            ...
            if (!user) continue; 
            if (user.SourceUserID && user.SourceUserID!=0 && user.UserID != user.SourceUserID) {
    ...
    for (var iTmp = 0; iTmp < len; iTmp++) {
        ...
        debugger;
        switch (AllData.IsSelectedCopyUser) {
            case "Selected":     //抄送默认选中 
                mini.get("lstCopyNode" + node.NodeCode).selectAll();
                break;
            case "SelectedAndLock":   //默认选中且不可撤销
                mini.get("lstCopyNode" + node.NodeCode).selectAll();
                mini.get("lstCopyNode" + node.NodeCode).setEnabled(false);
                mini.get("chkCopyNode" + node.NodeCode).setEnabled(false);
                break;
            case "NotSelected":   //默认别打钩
                break;
            default:
                mini.get("lstCopyNode" + node.NodeCode).selectAll();
                break;
    ...
    for (var iTmp = 0; iTmp < len; iTmp++) {
        ...
        var chkCopyBox = mini.get("chkCopyNode" + node.NodeCode);  //如果设置了不
        if (chkBox.SelectNodeMode =="SelectedAndDisabled" && chkCopyBox) chkCopyBox.setEnabled(false);
    ...
    if (chkCopyBox) {
        ...
        switch (AllData.IsSelectedCopyUser) {
            case "Selected":     //抄送默认选中 
                ...
                break;
            case "SelectedAndLock":   //全选且不可撤销
                listCopyBox.selectAll();
                listCopyBox.setEnabled(false);
                break;
            case "NotSelected":   //默认别打钩
...
var GetNodesUser = function (data,flag) {
    ...
    debugger;
    var div = $("#mindList");
    WorkFlowUtil.DrawMindList(div, data.HistoryMind,data.OtherMind, data.Agreelable, data.DisagreeLabel);
