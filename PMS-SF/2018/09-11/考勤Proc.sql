...
begin
	...
	declare @EndDate datetime
			...
			select distinct HumanId from SF_HumanCheckOnWork where DeptId in (select Id from PB_Department where ParentId in (select isnull(ParentId,'00000000-0000-0000-0000-000000000000') from PB_Department where Id=@DeptId) or Id=@DeptId ) and Month=@Month and Year=@Year --找到当前部门在考勤表里面的所有人员，有‘借调’，因此不能用masterid,用deptid
			open MyCur --打开游标
			...
			--第二个循环
			declare MyCur cursor for --建立名称为MyCur的游标
			select HumanId,CheckOnWorkType,day from SF_HumanCheckOnWork where DeptId in (select Id from PB_Department where ParentId in (select isnull(ParentId,'00000000-0000-0000-0000-000000000000') from PB_Department where Id=@DeptId) or Id=@DeptId ) and Month=@Month and Year=@Year --取循环数据行
			open MyCur --打开游标
			...
			WHILE(@@FETCH_STATUS = 0) --游标读取下一条数据是否成功
				...
				set @Total=@count2+@count3+@count4+@count5+@count6+@count7+@count8+@count9
