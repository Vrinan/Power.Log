...
--sum(InApplyCount) as InApplyCount,sum(InApplyAmout) as InApplyAmout,
...
--(select isnull(sum(c.RealApplyAmout),0) from PS_CM_ContractApplyMoney c where c.status=50 and a.Id = c.Contract_Guid) InApplyAmout,
