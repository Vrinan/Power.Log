    ...
    return mobilesingleself = { //执行初始化操作结束后 触发
        ...
        BuildHTML: function (data, htmlparams) {
            ...
            for (var j = 0; j < data.length; j++) {
                var row = data[j];
                html += " <li>" +
               "<a href=\"javascript:;\" " + mobilesingleself.GetOnClick(htmlparams, row) + ">" +
        ...
        GetOnClick: function (htmlparams, row) {
            ...
            if (htmlparams.formid != "") {
                onclick = "onclick=\"PowerForm.OpenWebForm('" + htmlparams.formid + "','" + row[htmlparams.idfield] + "')\"";
            }
            return onclick;
        },
        //获取行的【标题】html
        ...
        OpenWebForm: function (formid, keyvalue) {
            //if (typeof (window.m3app.AppCall) == "function") {
            //    window.m3app.AppCall('appShowTabbar', '{"enable":"0"}', function XXX(outparam) {
