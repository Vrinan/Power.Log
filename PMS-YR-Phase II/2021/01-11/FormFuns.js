...
function dealwithData(event) {
    ...
    for (var i = 0; i < data.rows.length; i++) {
        var row = data.rows[i];
        if (istree && selected == undefined)
            grid.clearSelect();
        //第二次复制时，如果是i的话，插入的数据顺序不对，所以改为0
        var newRow = funsself.OnGridTreeAdd(fileImportKeyWord, 0);
        //循环行中的每一列，并对应写成名称：值
...
function allheaderdata(columns, header, fields, parentHeader) {
    for (var i = 0; i < columns.length; i++) {
        var col = columns[i];
        if (col.columns && col.columns.length > 0)
            allheaderdata(col.columns, header, fields, parentHeader + col.header);
        if ($.trim(parentHeader + col.header) == $.trim(header)) {
...
var FormFuns = function () {
    ...
    return funsself = {
        ...
        onheadercellclick: function (e) {
            if (e.column.type != "indexcolumn" && e.column.type != "checkcolumn") {
                if (!e.column.allowSort) {
                    Power.ui.warning("本列不允许排序");//本列不允许排序
                    return;
                }
                var config = funsself.GetConfig(e.source.id, null);
                if (!config) {
                    Power.ui.warning("通过{0}找不到配置文件", { msgList: [e.source.id] });//通过xx找不到配置文件
                    return;
                }
        ...
        OnGridTreeAdd: function (gridid, index) {
            ...
            if (!grid) {
                Power.ui.warning("通过{0}找不到控件", { msgList: [gridid] });//通过xx找不到控件
                return;
            }
            ...
            if (funsself.IsMiniTree(grid)) {
                ...
                if (funsself.GetConfig(gridid).codefield != null && funsself.GetConfig(gridid).codefield != "") {
                    ...
                    if (childs.length == 0) {
                        //没有childs时，编号为上一个+.1
                        if (grid.getSelected() && grid.getSelected()[codefield])
                            newRow[codefield] = grid.getSelected()[codefield] + ".1";
                    }
                }
                if (grid.getSelected()) {
        ...
        //复制前
        EventBeforeCopy: function (e, ids) { },
        //复制后
        EventAfterCopy: function (e, ids) { },
        ...
        IsRowCanMove: function (grid, row, cfg) {
            if (row == undefined) {
                Power.ui.warning("请选中需要移动的行");//请选中需要移动的行
                return false;
            }
            if (cfg.sequfield == undefined || cfg.sequfield.length == 0) //没有序号字段，上下移动没有意义
            {
                Power.ui.warning("没有定义序号字段");//"没有定义序号字段"
                return false;
            }
        ...
        RowMoveUp: function (miniid) {
            ...
            if (funsself.IsMiniGrid(tree)) {
                ...
                if (index == 0) {
                    Power.ui.warning("第一行不能再上移");//"第一行不能再上移"
                    return false;
                }
            ...
            if (funsself.IsMiniTree(tree)) {
                ...
                if (!pnode) {
                    Power.ui.warning("树形结构异常");//"树形结构异常"
                    return false;
                }
        ...
        RowMoveDown: function (miniid) {
            ...
            if (funsself.IsMiniGrid(tree)) { //grid处理
                ...
                if (index == tree.data.length - 1) {
                    Power.ui.warning("最后一行不能再下移");//最后一行不能再下移
                    return false;
                }
                var cnode = tree.data[index + 1];
                if (!funsself.IsRowSequOK(cnode, cfg.sequfield, tree)) return false;
        ...
        RowMoveLeft: function (miniid) {
            ...
            if (funsself.IsMiniTree(tree) == false) {
                Power.ui.warning("{0}不是一棵树,不能左移", { msgList: [miniid] });//"不是一棵树,不能左移"
                return false;
            }
            var row = tree.getSelected();
            if (row == undefined) {
            ...
            if (tree.getLevel(row) == 0) {
                Power.ui.warning("根节点不能再左移");//"根节点不能再左移"
                return false;
            }
            var pnode = tree.getParentNode(row);
            if (!pnode) {
                Power.ui.warning("未找到父节点,不能左移");//"未找到父节点,不能左移"
                return false;
            }
            var ppnode = tree.getParentNode(pnode);
            if (!ppnode) {
                Power.ui.warning("未找到移动后的父节点,不能左移");//"未找到移动后的父节点,不能左移 "
                return false;
            }
        ...
        RowMoveRight: function (miniid) {
            var tree = mini.get(miniid);
            if (funsself.IsMiniTree(tree) == false) {
                Power.ui.warning("{0}不是一棵树,不能右移", { msgList: [miniid] });//"不是一棵树,不能右移"
                return false;
            }
            var row = tree.getSelected();
            if (row == undefined) {
            ...
            Power.ui.warning("末级节点或当前层级第一个节点不能右移");//"末级节点或当前层级第一个节点不能右移"
            return false;
        },
        //设置form可编辑属性
        ...
        GetUpdateValues: function (data, fields) {
            ...
            for (var tofd in fields) {
                if (typeof (fields[tofd]) == "function") continue;
                result[tofd] = data[fields[tofd]] || null;
            }
            return result;
        },
        ...
        //从formconfig中获取子级节点的config
        GetChildrenConfig: function (miniid, config) {
            if (!config)
                config = formconfig.config.joindata;
            if (miniid == config.miniid)
                return funsself.ConfigToList(config);
            var result = null;
            if (!config.children || config.children.length == 0)
                return result;
            for (var i = 0; i < config.children.length; i++) {
                ...
                if (c.miniid == miniid)
                    return c;
                result = funsself.GetChildrenConfig(miniid, c);
                if (result)
                    return result;
            }
        },
        ...
        InitComboboxData: function () {
            ...
            for (var item in comboboxdata) {
                ...
                cmbdata.Data = SetComboboxLanguageData(cmbdata.Data, cmbdata.TextField);
        ...
        FormReLoad: function (f, fncallback) {
            ...
            if (!config) {
                Power.ui.warning("通过{0}找不到配置文件", { msgList: [f.el.id] });//通过xx找不到配置文件
                return;
            }
            ...
            funsself.EventBeforeFormLoad(f, params);
            jQuery.ajax({
                url: "/Form/FormLoad",
                data: params,
                success: function (text) {
                    ...
                    if (!tmpdata.success) {
                        ...
                        Power.ui.error(tmpdata.message, { msgList: tmpdata.msgList, detail: tmpdata.detail, timeout: 0, position: "center center", closeable: true });
                    }
                    else {
                        ...
                        //尝试加载单页流程
                        if (typeof (WorkFlowsForm) != "undefined" && WorkFlowsForm) {
                            var workflowsForms = new WorkFlowsForm();
                            if (typeof (workflowsForms.LoadWorkFlows) == "function")
                                workflowsForms.LoadWorkFlows();
                        }
        ...
        LoadAttach: function (config) {
            ...
            if (mtabs != undefined) {
                ...
                //设置多附件页签
                var arrTabs = mtabs.getTabs();
                $.each(arrTabs, function () {
                    var thistab = this;
                    if (thistab.tabfor && thistab.tabfor == "attachfile") {
                        ...
                        mtabs.updateTab(thistab, upddata);
                        mtabs.reloadTab(thistab);
                        if (formconfig.KeyValue != "") {
                            var exp = { KeyWord: "DocFile", select: "Id" }
                            exp.swhere = "BOKeyWord='{0}' and FolderId='{1}'".format(mkeyword, formconfig.KeyValue);
                            if (this.filter) {
                                for (var i in thistab.filter) {
                                    exp.swhere += " and " + i + "='" + thistab.filter[i] + "'";
                                }
                            }
                            funsself.GridPageLoad(exp, function (o) {
                                if (o.success) {
                                    var t = mini.decode(o.data.value);
                                    var c = t == undefined ? 0 : t.length;
                                    var at = thistab.title;
                                    if (at.indexOf('(') > 0)
                                        at = at.substring(0, at.indexOf('('));
                                    var title = at + "(" + c + ")"
                                    mtabs.updateTab(thistab, { title: title });
                                }
                            })
                        }
                    }
        ...
        EventBeforeLoadCode: function (config) { },
        LoadCode: function (config, f) {
            var p = {};
            funsself.EventBeforeLoadCode(config);
        ...
        ReLoadData: function (o) {
            ...
            if (funsself.IsMiniForm(o))
                ...
                //用reload时会自动accept(),导致getChanges().Length为0无法触发befreload里的有修改未保存,是否保存
                var obj = o.getLoadParams();
                o.load(obj);
            }
        },
        //加载某个keyword的子表数据
        ...
        ValidData: function (miniid, bchild) {
            ...
            sender.validate()
            if ((typeof (sender.isValid) == "function" && sender.isValid() == false) || sender.isValid == false) {
                if (sender.getErrorTexts != undefined)
                    rlt.errortext += RequestTextToString(sender.getErrorTexts());
                else {
                    if (sender.title) {
                        rlt.errortext += ("[" + sender.title + "]" + "验证未通过".CC());
                    } else
                        rlt.errortext += (sender.id + "验证未通过".CC());
                }
                rlt.success = false;
                return rlt;
            }
        ...
        UpdateFormData: function (miniid, bchild) {
            var sender = funsself.GetMiniFormGrid(miniid);
            if (!sender) {
                Power.ui.warning("通过{0}找不到到Form/Grid/Tree", { msgList: [miniid] });//通过xx找不到到Form/Grid/Tree
                return;
            }
            var config = funsself.GetConfig(miniid);
            if (!config) {
                Power.ui.warning("通过{0}找不到配置文件", { msgList: [miniid] });//通过xx找不到配置文件
                return;
            }
        ...
        //生效，反生效操作
        OnFormEffect: function (e, actoin, formid) {
            //只有主表可以执行生效操作
            var senderid = funsself.GetGridTreeName(e.id);
            if (senderid != formconfig.config.joindata.miniid) {
                Power.ui.warning("只有主表才能执行生效操作{0}不是主表", { msgList: [senderid] });//"只有主表才能执行生效操作"+senderid+"不是主表"
                return;
            }
            ...
            $.ajax({
                url: url,
                type: "POST",
                data: { KeyWord: formconfig.config.joindata.KeyWord, KeyValue: formconfig.KeyValue, FormId: formid },
                cache: false,
                success: function (text) {
                    var o = mini.decode(text);
                    if (o.success == false) {
        ...
        //判断当是审批人打开表单,true 是审批人打开表单，false未知或不知道，为了在查看状态，如果是审批人打开表单也可编辑
        WorkFlowHandle: function () {
            //如果没有流程，返回false，不认为是审批人打开表单
            ...
            if (workflowdata.WorkFlowFlag && workflowdata.WorkFlowFlag == "workflows") {
                ...
                //是当前审批人，且设置了审批人可修改表单
                if (typeof (EUserType) != "undefined" && workflowdata.CanFlowOperate.UserType & EUserType.HandleMan > 0
                    && ((enableform != undefined && enableform == 1) || workflowdata.CanFlowOperate.IsAllowEditForm == "Allow"))
                    return true;
                else
                    return false;
            }
        ...
        WorkFlowUpdateForm: function () {
            ...
            if (enableform != undefined && enableform == 0 && config && config.currow && config.statusfield
                && config.currow[config.statusfield] != "0" && config.currow[config.statusfield] != "40"
                && (workflowdata.CurNodeType != 10 || !funsself.IsRegHuman(config, config.currow))
                && sessiondata.IsITAdmin == false && workflowdata && workflowdata.CanFlowOperate
                && workflowdata.CanFlowOperate.IsAllowEditForm != "Allow")
                return true;
            else
                return false;
        },
        ...
        LoadRight: function (miniid, bchild) {
            ...
            //Aladd
            //删除流程
            // if ($("#DelWorkflowBtn").length == 0 && formstate != "add") {
            //     $("#btnForward").parent().append("</i><a class='mini-button mini-state-default mini-corner-all blue' id='DelWorkflowBtn' onclick='DelWorkflow(" + config.KeyWord + ")'><i class='fa fa-trash-o'></i>删除流程</a>");
            //     if (sessiondata.HumanId.toLowerCase() == "ad000000-0000-0000-0000-000000000000" || sessiondata.HumanName=="王国栋") {
            //         $("#DelWorkflowBtn").show();
            //     } else {
            //         $("#DelWorkflowBtn").hide();
            //     }
            // }
            //编辑
            if($("#EditAllBtn").length==0){
                $("#btnForward").parent().append(" <a class='mini-button mini-state-default mini-corner-all blue' id='EditAllBtn' onclick='EditAll()'><i class='fa fa-pencil'></i>编辑</a>"); 
                if(sessiondata.HumanId.toLowerCase()=="ad000000-0000-0000-0000-000000000000" || sessiondata.HumanName=="王国栋") {
                    $("#EditAllBtn").show();
                }else{
                    $("#EditAllBtn").hide();
            ...
            //规定，若标签为CanEditAll,则编辑权限和新增一样
            if (typeof WFRight != "undefined" && WFRight[miniid] && typeof workflowdata != "undefined" && workflowdata.BookMarkCode) {
                var targetItem = null;
                for (var i = 0; i < WFRight[miniid].length; i++) {
                    if (WFRight[miniid][i].BookMarkCode == "CanEditAll") {
                        targetItem = WFRight[miniid][i];
                        break;
                    }
                }
                if (targetItem && ("," + workflowdata.BookMarkCode + ",").indexOf(",CanEditAll,") > -1 &&
            ...
            if (!oEvent.cancel && config && formstate && typeof (keywordright) != "undefined") {
                //新增、修改、删除设置按钮权限
                var fdRight = {
                    "bAdd": ["Add", "Add0", "Add1", "Add2", "AddForm", "AddFormChild", "AddFormBrother", "SaveAdd", "Copy"],
                ...
                if ($("#" + config.KeyWord + "\\.Import").length > 0) {
                    //判断是否有导入确认按钮，若有，则隐藏导入按钮
                    var btnName = "Import";
                    if ($("#" + config.KeyWord + "\\.ValidBeforeImport").length > 0) {
                        btnName = "ValidBeforeImport";
                        $("#" + config.KeyWord + "\\.Import").css("display", "none");
                    }
                    if (kwright && kwright.bEdit == "0") {//每个子表判断是否有导入权限
                        $("#" + config.KeyWord + "\\." + btnName).css("display", "none");
                    }
                }
                ...
                var wfisflowed = funsself.WorkFlowHandle();
                ...
                if ((formstate == "view" && wfisflowed == false) || effected == true || wfupdateform == true || wfreadonly == true || (!workflowdata && !kwright)) {
                    ...
                    if ($("#" + config.KeyWord + "\\.Import").length > 0) {//每个子表判断是否有导入权限
                        $("#" + config.KeyWord + "\\.Import").css("display", "none");
                    }
            ...
            //根据标签放开特定字段
            if (typeof WFRight != "undefined" && WFRight[miniid] && typeof workflowdata != "undefined" && workflowdata.BookMarkCode) {
                var array = WFRight[miniid];
                for (var i = 0; i < array.length; i++) {
                    var obj = array[i];
                    if (obj.BookMarkCode && ("," + workflowdata.BookMarkCode + ",").indexOf("," + obj.BookMarkCode + ",") > -1 &&
                        workflowdata.CanFlowOperate &&
                        (workflowdata.CanFlowOperate.CanFlowOperate & ECanFlowOperate["GetBack"]) == 0) {
                        if (obj.Btn && obj.Btn.length > 0) {
                            (obj.Btn).forEach(function (item) {
                                mini.get(miniid + "." + item).setEnabled(true);
                            });
                        }
                        if (obj.FormControl && obj.FormControl.length > 0) {
                            (obj.FormControl).forEach(function (item) {
                                mini.get(miniid + "." + item).setReadOnly(false);
                            });
                        }
                        if (obj.Column && obj.Column.length > 0) {
                            var grid = mini.get(miniid);
                            (obj.Column).forEach(function (item) {
                                grid.getColumn(item).readOnly = false;
                            });
                        }
                    }
                }
        ...
        LoadCustomColumn: function (title, formid, humanid) {
            ...
            if (!humanid) return;
            if (!title) title = "栏位".CC();

            var btns = $(".customcolumn");
            if (btns.length == 0) return;

            var builderhtml = function (btn, grdid) {
                var p = { KeyWord: "HumanCustom", select: "Id,Name,IsDefault,HumanId,IsPublic", sort: "Sequ" };
                p.swhere = "CustomType='WidgetColumn' and ObjectId='" + formid + "' and ObjectCode='" + grdid + "' and (HumanId='" + humanid + "' or IsPublic=1)";
                funsself.GridPageLoad(p, function (o) {
                    ...
                    for (var j = 0; j < dt.length; j++) {
                        ...
                        if (dt[j].IsDefault == "1" && dt[j].HumanId == sessiondata.HumanId) {
                            html += "<i class='fa fa-check'></i>";
                            defaultid = dt[j].Id;
                        }
                        if (dt[j].IsPublic == "1" && dt[j].HumanId != sessiondata.HumanId)
                            html += ("(全局)".CC() + dt[j].Name + "</a></li>");
                        else
                            html += (dt[j].Name + "</a></li>");
                    }
                    html += "<li class='divider'></li>";
                    html += "<li class='Actived'><a class='btn' onclick=\"FormFuns.SetCustomColumn('" + formid + "','" + humanid + "','" + grdid + "')\"'>" + "栏位定义".CC() + "</a></li></ul>";
                    $(btn).html(html);
                    if (defaultid) {
        ...
        ChangeCustomColumn: function (sender, gridid, customid) {
            ...
            funsself.GridPageLoad({ KeyWord: "HumanCustom", select: "ExtJson", swhere: "Id='" + customid + "'" }, function (o) {
                ...
                for (var i = 0; i < ext.length; i++) {
                    ...
                    if (ext[i].sort == "asc" || ext[i].sort == "desc") {
                        arry.push({ id: (parseInt(ext[i].sortsequ) || 0), value: ext[i].field + " " + ext[i].sort });
                    }
                }
                //按级别排序
                arry = arry.sort(function (a, b) { return a["id"] - b["id"]; });
                //获取sql排序
                for (var i = 0; i < arry.length; i++) {
                    if (_customSequ != "")
                        _customSequ += ",";
                    _customSequ += arry[i]["value"];
                }
                grid._customSequ = _customSequ;
                //设置列排序
                for (var i = 0; i < ext.length; i++) {
                    for (var j = 0; j < cols.length; j++) {
                        if (cols[j].columns && cols[j].columns.length > 0) {

                        } else if (cols[j].field == ext[i].field) {
                            cols[j].sort = i;
                        }
                    }
                }
                funsself.SortCustomColumn(cols);
                grid.setColumns(cols);
                grid.reload();
                var cs = $(".customcolumn")
                for (var i = 0; i < cs.length; i++) {
                    if ($(cs[i]).attr("gridid") == grid.id) {
                        $($($(cs[i]).children()[0]).children()[0]).remove();
                        $($(cs[i]).children()[0]).prepend("<i class='fa fa-check' style='color:red'></i>")
                    }
                }
        ...
        SortCustomColumn: function (cols) {
            cols.sort(function (a, b) {
                return a.sequ - b.sequ;
            });
            for (var i = 0; i < cols.length; i++) {
                if (cols[i].columns && cols[i].columns.length > 0) {
                    funsself.SortCustomColumn(cols[i].columns);
                }
            }
        },
        GetCustomColumnVisiable: function (ext, cols) {
            for (var j = 0; j < cols.length; j++) {
                var col = cols[j];
                if (col.columns && col.columns.length > 0) {
                    if (col.header == ext.header) {
                        col.sequ = ext.sequ;
                    }
                    funsself.GetCustomColumnVisiable(ext, col.columns);
                }
                else {
                    if (col.field && col.field == ext.field) {
        ...
        SetCustomColumn: function (formid, humanid, gridid) {
            ...
            mini.open({
                ...
                onload: function () {
                    ...
                    //}
                    var recolumns = [];
                    funsself.ResetCustomColumn(grid.columns, recolumns);
                    data.column = recolumns;
        ...
        ResetCustomColumn: function (columns, returncolumns) {
            for (var i = 0; i < columns.length; i++) {
                if (columns[i].type == "indexcolumn" || columns[i].type == "checkcolumn")
                    continue;
                var col = {};
                col.id = columns[i]._id;
                col.pid = columns[i]._pid;
                col.field = columns[i].field;
                col.header = columns[i].header;
                col.bvisible = columns[i].visible;
                col.iwidth = columns[i].width.replace('px', '');
                returncolumns.push(col);
                if (columns[i].columns && columns[i].columns.length > 0) {
                    funsself.ResetCustomColumn(columns[i].columns, returncolumns);
                }
            }
        },
        ...
        SetCustomFilter: function (formid, humanid, gridid) {
            ...
            mini.open({
                ...
                showMaxButton: true,
                onload: function () {
                    var data = { formid: formid, humanid: humanid, gridid: gridid, keyword: config.KeyWord, keywordType: config.KeyWordType, fields: {} };
                    if (grid._customfilterid)
                        data._customfilterid = grid._customfilterid;
                    var cols = [];
                    funsself.GetTreeGridAllHeaders(grid.columns, cols);
                    for (var i = 0; i < cols.length; i++) {
                        var col = cols[i];
        ...
        GridPageLoad: function (p, fncallback) {
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    if (!o.success)
                        Power.ui.error(o.message, { msgList: o.msgList, detail: o.detail });
                }
            })
        },
        //打开页面执行新增数据操作
        OnAddForm: function (formid, defaultdata, grid, params) {
            ...
            var iw = formconfig.config.openwidth || 1250;
            var ih = formconfig.config.openheight || 700;
            ...
            if (opentype.toLowerCase() == "div") {
                var t = mini.open({
                    ...
                    ondestroy: function () {
                        ...
                        if (!iframe.contentWindow.formconfig || !iframe.contentWindow.formconfig.config ||
                            !iframe.contentWindow.formconfig.config.joindata ||
                            !iframe.contentWindow.formconfig.config.joindata.KeyWord)
                            return;
                        ...
                        if (b) {
                            ...
                            buttonOption[yes] = {
                                ...
                                handler: function (ret) {
                                    ...
                                    iframe.contentWindow.PowerForm.OnBtnSave({ id: iframe.contentWindow.formconfig.config.joindata.KeyWord + ".Save" }, false, null, null, function (success) {
                                        if (success) {
                            ...
                            buttonOption[cancel] = function () {
                                ifwin.autocancel = false;
                            }
                            ifwin.Power.ui.confirm("有修改未保存,是否保存", { button: buttonOption });
                            if (!ifwin.autocancel) {
                                return false;
                            }
                        }
        ...
        OnCloseCancel: function (form) {
            ...
            for (var field in pack) {
                ...
                if (typeof (form.formconfig.config.joindata.oldcurrow[field]) == "undefined")
                    continue;
                ...
                //直接使用isNaN可判断是不是数字，parseFloat会导致字符串："0819aaa"截取为了819
                if (!isNaN(pack[field].toString()) && !isNaN(oldf.toString())) {
        ...
        OnEditViewForm: function (btn, formid, action, params) {
            ...
            if (!row) {
                Power.ui.warning("请选中要打开的行");//请选中要打开的行
                return;
            }
            //按钮设置独立的openformid
            ...
            funsself.OnOpenForm(p, function () {
                ...
                if (getParameter("token") != "" && typeof (sysapphtml) != "undefined") {
        ...
        OnOpenForm: function (e, fncallbak) {
            ...
            //如果有token，认为是app端打开，url需要改为其他
            var showMaxButton = true;
            if (getParameter("token") != "" && typeof (sysapphtml) != "undefined") {
                url = "/Mobile/ValidForm/" + e.formid + "/" + e.action + "/" + e.keyvalue + "/" + base64encode(mini.encode(e.params)) + "?token=" + getParameter("token");
                showMaxButton = false;
            }
                        ...
                        win = mini.open({
                            ...
                            ondestroy: function () {
                                ...
                                if (b) {
                                    ...
                                    iframe.contentWindow.Power.ui.confirm("有修改未保存,是否保存", { button: buttonOption });
        ...
        MiniOpenURL: function (url, p) {
            ...
            if (!p.buttons)
                p.buttons = [{ html: '<a class="btn btn-xs blue btnclose" style="position:relative;top:-4px;" onclick="$(this).parents(\'.mini-panel\').remove();$(\'.mini-modal\').remove()">' + "关闭".CC() + '</a>' }];//关闭
        ...
        SendNotify: function (e) {
            ...
            if (e.showconfirm == undefined)
                e.showconfirm = true;
            e.confirmtitle = e.confirmtitle || "确定发送通知消息?".CC();
            var dosend = function () {
                $.ajax({
        ...
        GetExportXlsCol: function (cols) {
            ...
            for (var i = 0; i < temp.length; i++) {
                ...
                if (item.type == "comboboxcolumn" && item.editor) {
                    ...
                    var tCmbData = item._data || item.editor.data;
                    node.comboboxdata = [];
                    if (tCmbData) {
                        for (var j = 0; j < tCmbData.length; j++)
                            node.comboboxdata.push({ id: tCmbData[j][idfd], text: tCmbData[j][txtfd] });
                    }
                }
        ...
        ExportToXls: function (gridid, xtype, filename, fnonbeforeload, sort) {
            ...
            var p = { sender: tab, params: { pageIndex: 0, sizeIndex: 0 }, source: tab.source };
            ...
            if (tab._customSequ) {
                json.Order = tab._customSequ;
            }
            debugger;
            json.excelColumns = JSON.stringify(excelColumns);
            json.MenuId = Power.cookie.get("menuids") == null ? "" : Power.cookie.get("menuids");
            if (tab.merge)
                json.mergeColumns = tab.merge;

            if (funsself.IsMiniTree(tab)) {

                var treeColumn = tab.getTreeColumn();
                if (treeColumn != "") {
                    json.treefield = tab.getColumn(treeColumn).field;
                }
                else
                    json.treefield = tab.getTextField();
                json.levelcolor = "{ 0: \"149, 202, 252\", 1: \"206, 255, 206\", 1: \"211, 211, 255\", 2: \"255, 238, 221\", 3: \"255, 238, 221\", 4: \"255, 255, 206\", 5: \"232, 232, 221\", 6: \"241, 241, 226\" }";
                json.fieldcolor = "";
                json.idfield = tab.idField;
                json.parentfield = tab.parentField;
            }
            if (typeof (swherereplace) != "undefined" && swherereplace.length > 0) {
                json.Where = funsself.ReplaceCondition(json.Where, swherereplace);
            }
            var encodejson = encodeURIComponent(mini.encode(json));
            postDataToServer(sys_powerExportSite + "/Form/ExportFile?cookieId=" + SiteCookieId, { json: encodejson, KeyWordType: keywordtype });
        },
        //导出到excel, grid固定data的数据
        ExportDataToXls: function (gridid, filename, groupfield, fnonbeforeload, headercolor, headerfontcolor, exporttype, levelcolor, fieldcolor) {
            var tab = mini.get(gridid);
            if (tab == undefined) {
                Power.ui.warning("通过{0}没有找到Grid", { msgList: [gridid] });
                return;
            }
            var p = { sender: tab };
            if (fnonbeforeload) fnonbeforeload(p);
            ...
            var url = sys_powerExportSite + "/Form/ExportPageData?cookieId=" + SiteCookieId;
            if (funsself.IsMiniTree(tab)) {
                ...
                if (treeColumn != "") {
                    ...
                    json.treefield = tab.getTextField();
                json.levelcolor = levelcolor == null ? "{ 0: \"149, 202, 252\", 1: \"206, 255, 206\", 1: \"211, 211, 255\", 2: \"255, 238, 221\", 3: \"255, 238, 221\", 4: \"255, 255, 206\", 5: \"232, 232, 221\", 6: \"241, 241, 226\" }" : levelcolor;
                json.fieldcolor = fieldcolor == null ? "" : fieldcolor;
                url = sys_powerExportSite + "/Form/ExportTreePageData?cookieId=" + SiteCookieId;
            }
            //如果有numberformat，将第一行的字段转为numberformat对应的小数，否则第一行是整数的话，转成DataTable这一列都会变成整数
            if (data.length > 0) {
                var RecursiveFunc = function (excelColumns) {
                    for (var i = 0; i < excelColumns.length; i++) {
                        var exc = excelColumns[i];
                        if (exc.children && exc.children.length > 0)
                            RecursiveFunc(exc.children);
                        else if (exc.numberformat) {
                            for (var j = 0; j < data.length; j++) {
                                var dataV = data[j][exc.field];
                                if (mini.isNumber(dataV))
                                    data[j][exc.field] = mini.formatNumber(dataV, exc.numberformat);
                                else if (mini.isNumber(parseFloat(dataV)))
                                    data[j][exc.field] = mini.formatNumber(parseFloat(dataV), exc.numberformat);
                            }
                        }
                    }
                }
                RecursiveFunc(excelColumns);
            }
            //如果有分组
            ...
            //json.DateColumnFormat = "yyyy-MM-dd";
            ...
            var encodejson = encodeURIComponent(mini.encode(json));
            if (exporttype == "pdf") {
                postDataToServer(url, { json: encodejson }, "_blank");
            }
            else
                postDataToServer(url, { json: encodejson });
        },
        ExportTreeDataToXls: function (gridid, filename, treefield, groupfield, fnonbeforeload, headercolor, headerfontcolor, exporttype) {
            var tab = mini.get(gridid);
            if (tab == undefined) {
            ...
            if (exporttype == "pdf")
                json.PDF = "1";
            json.excelColumns = JSON.stringify(excelColumns);
            json.treefield = treefield;
            json.MenuId = Power.cookie.get("menuids") == null ? "" : Power.cookie.get("menuids");
            if (fnonbeforeload) fnonbeforeload(json)

            var encodejson = encodeURIComponent(mini.encode(json));
            if (exporttype == "pdf") {
                postDataToServer(sys_powerExportSite + "/Form/ExportTreePageData?cookieId=" + SiteCookieId, { json: encodejson }, "_blank");
            }
            else
                postDataToServer(sys_powerExportSite + "/Form/ExportTreePageData?cookieId=" + SiteCookieId, { json: encodejson });
        },
        ...
        PostFavorite: function (e) {
            ...
            $.ajax({
                ...
                success: function (text) {
                    ...
                    if (e.showmessage) {
                        if (!o.success)
                            Power.ui.error(o.message, { msgList: o.msgList });
                        else {
        ...
        CopyExcel: function (config) {
            ...
            if (grid != undefined && funsself.IsMiniGridTree(grid)) {
            ...
            if (formconfig) {
                for (var f in formconfig) {
                    ...
                    if (config && config.type && config.type.toLowerCase() == "excel") {
                        var btntext = "<i class=\"fa fa-upload\"></i>" + "导入Excel".CC();
                        var ob = $("#" + f.replace(".", "\\."));
                        ...
                        $("#" + f.replace(".", "\\.")).uploadify({
                            ...
                            batfiles: 0,//批量上传最大文件数
                            "buttonText": btntext,
                            "swf": '/Scripts/plugins/uploadify/swfupload.swf',
                            "uploader": sys_powerUploadFileASHX("action=upload&uploadtype=ExcelImportService_System"),
                            //headers: {
                            //    "token": SiteCookieId,
                            ...
                            formData: function () {
                                ...
                                rec.config = base64encode(JSON.stringify(config));
                                //找到所有的下拉框数据源，进行传递
                                if (comboboxdata && comboboxdata != "")
                                    rec.comboboxdata = base64encode(JSON.stringify(comboboxdata));
                                else
                                    rec.comboboxdata = "";
                                return rec;
                            },
                            onUploadStart: function () {
                                if ($(".modal-backdrop") && $(".modal-backdrop").css("display") != "block")
                            ...
                            onUploadComplete: function (file, responseText, fileId) {
                                ...
                                var e = { "id": this.config.miniid, "canNext": true, "isSuccess": true };
                                funsself.EventAfterImport(e);
                                if (e.canNext)
                                    mini.get(this.config.miniid).reload();
                            },
                            onUploadError: function (file, responseText) {
                                Power.ui.loading.close();
                                var e = { "id": this.config.miniid, "canNext": true, "isSuccess": false };
                                funsself.EventAfterImport(e);
                                if (e.canNext)
                                    mini.get(this.config.miniid).reload();
                            }
        ...
        GetSearchFieldsWhere: function (miniid) {
            ...
            if (property[field] != undefined && property[field] == "DateTime" && (value != "" || edtvalue_d.getFormValue())) {
                ...
                if (typeof (sqlType) == "string" && sqlType.toLowerCase() == "oracle") {

                    if (value)
                        swhere = swhere + " and " + field + " >= to_date('" + value + "','yyyy-MM-dd hh24:mi:ss')";
                    if (value2)
                        swhere = swhere + " and " + field + " <= to_date('" + value2 + " 23:59:59','yyyy-MM-dd hh24:mi:ss')";
                }
                else {
                    if (value)
        ...
        OnButtonEditClose: function (e) {
            //弹出框的关闭按钮事件，按钮id必须是[关键词.字段名]
            debugger;
            var obj = e;
            if (e.sender)
                obj = e.sender;
            //获取按钮id
            var miniid = funsself.GetGridTreeName(obj.id);
            var field = funsself.GetControlName(obj.id);
            var config = formconfig[obj.id];
            var kconfig = funsself.GetConfig(miniid);
            var sender = funsself.GetMiniFormGrid(miniid);
            //如果是form，改对应config的值
            if (kconfig && kconfig.currow && funsself.IsMiniForm(sender)) {
                obj.setText("");
                obj.setValue("");
                var fields = config.fields;
                fields = mini.decode(fields);
            ...
            //如果是grid，updateRow
                ...
                var updvalue = funsself.GetUpdateValues({}, config.fields);
                if (funsself.IsMiniGrid(sender))
                    sender.updateRow(row, updvalue);
                if (funsself.IsMiniTree(sender))
                    sender.updateNode(row, updvalue);
                //grid/tree中，buttonedit不会自动修改
                if (e.type != undefined && e.type == "closeclick" && funsself.IsMiniGridTree(sender)) {
        ...
        OnBtnCopy: function (id) {
            //1.如果有打开表单，优先以表单配置文件为基础复制，如果仅窗体，则以窗体配置文件为基础复制
            var formid = formconfig.FormId;
            if (formconfig.config.openformid)
                formid = formconfig.config.openformid;
            if (!formid) {
                Power.ui.warning("复制时，配置文件必须有formid或openformid");
                return;
            }
            //2.传递当前主键值、openformid，并调用复制方法 
            //如果id是主关键词，则直接传formid，否则按照前台传递config
            var config = "";
            if (id != formconfig.config.joindata.KeyWord) {
                config = funsself.GetChildrenConfig(id);
            }
            if (config)
                config = mini.encode(config);
            var sender = funsself.GetMiniFormGrid(id);
            if (!sender) {
                Power.ui.warning("通过" + id + "找不到到Form/Grid/Tree");//通过xx找不到到Form/Grid/Tree
                return;
            }

            var rows = null;
            var idfield = sender.getIdField();
            if (!idfield) {
                Power.ui.warning("复制时，tree/grid的主键字段");
                return;
            }
            if (funsself.IsMiniGrid(sender)) {
                var grid = sender;
            ...
            funsself.EventBeforeCopy(sender, ids);
            var buttonOption = {};
            var yes = "是";
            var no = "否";
            buttonOption[yes] = {
                ...
                handler: function (ret) {
                    if (ret) {
                        Power.ui.loading("复制中……");
                        $.ajax({
                            url: "/Form/CopyRecords",
                            type: "POST",
                            data: { formid: formid, ids: mini.encode(ids), config: config },
                            cache: false,
                            success: function (text) {
                                Power.ui.loading.close();
                                var tmp = mini.decode(text);
                                if (tmp.success) {
                                    Power.ui.success("复制成功");
                                    sender.reload();
                                }
                                else {
                                    Power.ui.error(tmp.message, { msgList: tmp.msgList });
                                }
                                funsself.EventAfterCopy(sender, ids);
                            }
                        })
                    }
                }
            ...
            buttonOption[no] = function () { };
            top.Power.ui.confirm("是否复制选中的记录", { button: buttonOption });

        },
        ReplaceCondition: function (where, condition) {
            //替换where条件
            for (var i = 0; i < condition.length; i++) {
                var cd = condition[i].split(":");
                if (cd.length != 2)
                    continue;

                where = where.split(cd[1]).join(cd[0]);
            }
            return where;
        }
    }
...
var EditAll = function(){
    //主表
    Power.ui.info("当前登录:"+sessiondata.HumanName+"，最高编辑权限");
...
// AlroyAdd-20201220
//打开表单数据，默认查看方式，
var ViewFormComn = function (formId, keyValue, openType, params) {
    var type = openType == undefined ? "view" : openType;
    var openUrl = "";
    params = params == undefined ? "" : params;

    if(type == "widget"){
        openUrl = "/Form/EditForm/" + formId + "/" + params
    }else{
        openUrl = "/Form/ValidForm/" + formId + "/" + type + "/" + keyValue + params;
    }
    var win = mini.open({
        url: openUrl,
        showMaxButton: true,
