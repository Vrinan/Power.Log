update XLX_PUR_Inquiry_MatList set XLX_PUR_Inquiry_MatList.Bomfile_Id = 
(select MasterId from PS_PUR_BOM where id = (select bomid from XLX_PUR_Request_List where id=XLX_PUR_Inquiry_MatList.RequestList_Id))
...
select * from View_XLX_PUR_Request

sp_helptext View_XLX_PUR_Request
...
alter VIEW [dbo].[View_XLX_PUR_Request]
AS
SELECT   main.Id AS MainId, main.Status, main.RequestCode, main.Title, main.ProMajor, main.RequestDate, main.IsUrgency, 
                ...
                list.UnitName, list.ProMajor AS ListProMajor, list.SpecFileId, list.SpecFileNo, list.SpecFileName, list.Remark,list.CBS_Name,XLXOrder.OrderCode, 
                ...
                list.BackName, list.BackTime,list.CBS_Id,list.CBSPath,CASE when sub.XLX_Request_List_Id IS NULL or sub.XLX_Request_List_Id='00000000-0000-0000-0000-000000000000' 
				THEN '未生成合同' when sub.XLX_Request_List_Id IS NOT NULL AND sub.XLX_Request_List_Id<>'00000000-0000-0000-0000-000000000000' THEN '生成合同' END SubContractStatus,
...
FROM      dbo.XLX_PUR_Request AS main INNER JOIN
                dbo.XLX_PUR_Request_List AS list ON main.Id = list.FK LEFT OUTER JOIN
                    ...
                    (SELECT   main.OwnProjId, main.OwnProjName, main.InquirySupplieStatus, list.XLX_Request_List_Id
                     ...
                     WHERE   (ISNULL(main.InquirySupplieStatus, N'') = '竞价方案已生成' or ISNULL(main.InquirySupplieStatus, N'') = '竞价方案已批准')) AS PSInquiry ON list.Id = PSInquiry.XLX_Request_List_Id
					  LEFT OUTER JOIN
                    (SELECT   main.OrderCode, list.InquiryMatList_Id
                     FROM      dbo.XLX_PUR_Order AS main INNER JOIN
                                     dbo.XLX_PUR_Order_list AS list ON main.Id = list.FK ) AS XLXOrder ON list.Id = XLXOrder.InquiryMatList_Id
                ...
                list.UpdDate, list.TaskStatus, list.SenderId, list.SenderName, list.SendTime,list.MatBS_Code,list.BuyerId, list.BuyerName, list.BackId, 
                list.BackName, list.BackTime, Inquiry.InquirySupplieStatus,list.CBS_Id,list.CBSPath,sub.XLX_Request_List_Id,PSInquiry.InquirySupplieStatus,bomfile.Id
