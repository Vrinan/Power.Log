...
BEGIN
	...
	declare @ContractCode nvarchar(100)
	...
	select  
			...
			@Storage_Guid=Storage_Guid,@StorageName=StorageName,@SupplierId=SupplierId,@Supplier=Supplier,@ContractCode=ContractName
		...
		begin	
			...
			if(@Count2=0)
				...
				begin
					...
					if(len(@Code1)=1)
						begin
							set @InStoreCode = 'WZRK-'+@date+'-'+'00'+ @Code1
						end
					if(len(@Code1)=2)
						begin
							set @InStoreCode = 'WZRK-'+@date+'-'+'0'+ @Code1
						end
					if(len(@Code1)=3)
						begin
							set @InStoreCode = 'WZRK-'+@date+'-'+ @Code1
						end
				end

				set @NewId=NEWID()
				insert into PS_PUR_MatInStore(Status,EpsProjId,OwnProjId,OwnProjName,RegHumId,RegHumName,RegDate,UpdDate,Id,Memo,
					Arrival_Guid,ArrivalCode,Storage_Guid,StorageName,InStoreDate,InStoreFrom,code,SubContract_Guid,ContractCode) 
				values(0,@OwnProjId,@OwnProjId,@OwnProjName,@RegHumId,@RegHumName,GETDATE(),GETDATE(),@NewId,@Other,
					@Id,@Code,@Storage_Guid,@StorageName,GETDATE(),'yi',@InStoreCode,@Contract_Guid,@ContractCode)
				 
				insert into PS_PUR_MatInStore_Dtl(Id,MasterId,Mat_Guid,ArrivalMat_Guid,Sequ,MatCode,MatName,MatDescription,MatUnit,UnitPrice,InStoreNum,InStoreAmount,Freight,TotalAmount,BatchCode,UpdDate,XLX_RequestId,MatSpec,TagNumber,Remarks,Specifi,Standard,
				Texture,Pattern)
				select
					NEWID(),@NewId,Mat_Guid,Id,Sequ,MatCode,MatName,MatDescription,MatUnit,UnitPrice,ArrivalNum,UnitPrice*ArrivalNum,0,0,'',GETDATE(),XLX_RequestId,MatSpec,TagNumber,Remarks,Specifi,Standard,Texture,Pattern
